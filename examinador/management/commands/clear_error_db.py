# examinador/management/commands/clear_db.py
# Se usa el comando 'python manage.py clear_error_db'

#os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'Opositorium.settings') 
#django.setup()

from django.core.management.base import BaseCommand
from django.db import connection  # Para ejecutar consultas SQL directas
from examinador.models import ErrorReport

class Command(BaseCommand):
    help = "Vacía la base de datos de preguntas y reinicia el contador de ID. Se usa el comando 'python manage.py clear_error_db'"

    def handle(self, *args, **options):
        self.stdout.write(self.style.WARNING('¿Estás seguro de que quieres vaciar la base de datos de reportes de errores? (s/n)'))
        confirmacion = input()

        if confirmacion.lower() == 's':
            # Eliminar todos los registros
            ErrorReport.objects.all().delete()

            # Reiniciar el contador de ID para la tabla
            with connection.cursor() as cursor:
                cursor.execute("DELETE FROM sqlite_sequence WHERE name='errores_reportados';")

            self.stdout.write(self.style.SUCCESS('Base de datos vaciada exitosamente y contador de ID reiniciado.'))
        else:
            self.stdout.write(self.style.INFO('Operación cancelada.'))